...
export default function MapView({ onLocationSelect, reports = [], center, zoom = 13 }: MapViewProps) {
  ...
  // Combiner les données des props et les données en temps réel
  const allReports = useMemo(() => {
    // Convertir les données en temps réel au format UnifiedReport
    const realtimeUnifiedReports: UnifiedReport[] = realtimeReports.map(route => ({
      id: route.routeId,
      ...
      longueurKm: route.longueurKm,
      surfaceM2: route.surfaceM2,
      budget: route.budget,
      entreprise: route.entreprise,
      dateCreation: route.dateCreation,
      dateMiseAJour: route.dateMiseAJour
    }));
    
    // Combiner avec les données des props
    return [...realtimeUnifiedReports, ...reports];
  ...
  useEffect(() => {
    ...
    const unsubscribeRoutes = syncService.listenToRouteUpdates((routes: RouteEndommagee[]) => {
      ...
      // Les marqueurs seront mis à jour automatiquement via allReports
    });
    ...
    const unsubscribeSystem = syncService.listenToSystemNotifications((notification) => {
      ...
      if (notification.source === 'mobile-app' && notification.action === 'new-report') {
        ...
        // Les données seront mises à jour via le listener routes
      }
    });
  ...
  useEffect(() => {
    console.log('MapView useEffect - Nombre de reports:', allReports.length);
    ...
    console.log('Début du traitement des', allReports.length, 'reports');
        ...
        .bindPopup(`
          <div style="min-width: 250px;">
            ...
            <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #e5e7eb;">
              <p style="margin-bottom: 4px; font-size: 12px;"><strong>Date:</strong> ${new Date(report.dateCreation || report.created_at || new Date()).toLocaleDateString('fr-FR')}</p>
              <p style="margin-bottom: 4px; font-size: 12px;"><strong>Status:</strong> <span style="color: ${color}; text-transform: capitalize;">${getStatusText(report.statut || report.status || 'nouveau')}</span></p>
  ...
  }, [allReports, selectedLocation, onLocationSelect]);
